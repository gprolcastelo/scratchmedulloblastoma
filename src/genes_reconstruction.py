...
save_path_rn = f"reports/figures/{today}_reconstruction_network_wasser/"
...
df_net_output_test.shape

plt.figure(figsize=(10, 6))
plt.hist(reconstruction_x_test.flatten(), bins=100, alpha=0.5, label='VAE output', color='cyan', edgecolor='black')
plt.hist(X_test.values.flatten(), bins=100, alpha=0.5, label='original', color='orange', edgecolor='black')
plt.hist(net_output_test.detach().numpy().flatten(), bins=100, alpha=0.5, label='Network output', color='cornflowerblue', edgecolor='black')
plt.legend()
plt.savefig(os.path.join(save_path_rn,'histogram_reconstruction_network.svg'),format='svg',bbox_inches='tight')
plt.savefig(os.path.join(save_path_rn,'histogram_reconstruction_network.png'),format='png',dpi=600,transparent=True,bbox_inches='tight')
plt.savefig(os.path.join(save_path_rn,'histogram_reconstruction_network.pdf'),format='pdf',bbox_inches='tight')
plt.show()
...
w_dists_genes_train_mean.shape, w_dists_genes_train_std.shape
